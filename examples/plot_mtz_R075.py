"""Figura comparativa dos-fases vs MTZ conjunto (cuadrado 4x4, R=0.75)."""
import sys
...
print(f"MTZ: k={j.get('k')} L={round(j.get('length', float('nan')), 4)} "
...
for a, C, order, ttl in ((ax[0], C2, order2,
                         ...
                         (ax[1], Cj, orderj,
                          f"MTZ conjunto k={len(Cj)} L={j['length']:.2f}")):
    a.set_aspect("equal")
    ...
    a.legend(loc="best")
plt.tight_layout()
out = str(Path(__file__).resolve().parent / "simultaneous_4x4_R075.png")
