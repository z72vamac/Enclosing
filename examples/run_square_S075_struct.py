"""Cuadrado 4x4, cuadrados s=0.75 con semillas estructurales (vx+/-s)."""
import sys
...
res = iterative_cover(poly, R, h0=R / 2, s0=R, max_iter=4, verbose=True,
                      time_limit=120, shape="square", seed="structural")
...
out = Path(__file__).resolve().parent / "square_S075_struct.png"
plot_solution(poly, res["centers"], R, demands=None, save=str(out),
              title=f"Cuadrado 4x4 s=0.75 estruct k={res['k']}", shape="square")
print("figura:", out)
